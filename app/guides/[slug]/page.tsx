...
export default async function GuidePage({params}:{params:Promise<{slug:string}>}){
  ...
  return <main className="lab-article">
    ...
    <nav className="crumbs" aria-label="Breadcrumb"><Link href="/">Element Lab</Link><span>/</span><span>{guide.category}</span></nav>
    <header className="article-lead"><p className="eyebrow">Lab note · {guide.updated}</p><h1>{guide.title}</h1><p className="dek">{guide.description}</p></header>
    <section className="result-card"><span>Reproducible starting point</span><p>{guide.quickAnswer}</p></section>
    <GuideAd/>
    ...
    <div className="notebook"><aside><span>Field sequence</span>{guide.sections.map((section,index)=><a key={section.heading} href={`#step-${index+1}`}>{String(index+1).padStart(2,"0")} {section.heading}</a>)}</aside><div className="notebook-pages">{guide.sections.map((section,index)=><Fragment key={section.heading}><section id={`step-${index+1}`}><div className="step-marker">{String(index+1).padStart(2,"0")}</div><h2>{section.heading}</h2>{section.paragraphs.map((paragraph)=><p key={paragraph}>{paragraph}</p>)}{section.visual&&section.visual.assetId!==leadVisual?.assetId?<figure data-asset-id={section.visual.assetId}><Image src={section.visual.src} width={section.visual.width} height={section.visual.height} sizes="(max-width: 900px) 100vw, 760px" alt={section.visual.alt}/><figcaption>{section.visual.caption}</figcaption></figure>:null}</section></Fragment>)}<section className="sources"><h2>Sources &amp; further checks</h2>{guide.sources.map((source)=><p key={source.url}><a href={source.url} rel="noreferrer">{source.label} ↗</a></p>)}</section></div></div>
  </main>;
}
